...
(function(){
  var combo, arenaSource, clockElem, boardSize, cellSize, rightMargin, legendPos, arenaElem, paper, testData, drawLine, robotColors, nextColor, idsToColor, getRobotColor, draw;
  ...
  draw = function(data){
    var i$, to$, x, y, ref$, len$, index, robot, pos, color, results$ = [];
    ...
    for (i$ = 0, len$ = (ref$ = data.positions).length; i$ < len$; ++i$) {
      ...
      results$.push(paper.text(legendPos[0] + 1.5 * cellSize, (index * 2 + 0.5) * cellSize, robot.id).attr({
        "font-size": "16px",
        "text-anchor": "start"
      }));
    }
